...
const getData = async () => {
    try {
        ...
        const response = await getBumdesUnits({
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <div class="card-title">
                        <h5 class="fw-bold">Unit Usaha Bumdes</h5>
                    </div>
                    <div class="card-tools" v-if="isOperator">
                        <RouterLink :to="{name: 'unit-usaha-create'}" class="btn text-white" style="background: #2F4F4F;"><i class="fas fa-plus"></i> Tambah
                            Data</RouterLink>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <div class="table-scroll">
                        <table class="table table-striped table-bordered">
                            ...
                            <tbody>
                                <tr v-if="bumdesunit.length > 0" v-for="(item, index) in bumdesunit" :key="item.id">
                                    ...
                                    <td>
                                        <span
                                            :class="item.status === 'active' ? 'badge bg-success' : 'badge bg-warning'">{{
                                            item.status }}</span>
                                    </td>
                                    <td width="18%" v-if="isOperator">
                                ...
                                <tr v-else>
                                    <td colspan="7" class="text-center text-muted py-4">
                                        ...
                                        <p class="mb-5">{{ isSearching ? 'Data tidak ditemukan' : 'Belum ada data!' }}
                                        </p>
                                    </td>
                                </tr>
                            </tbody>
